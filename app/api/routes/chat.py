...
# In-Memory 세션 저장소 (서버 재시작 시 초기화됨), 개선점: 원래는 디비에 저장해야 됌
...
async def stream_with_status(
    ...
    async for mode, event in graph.astream(
        ...
        # LLM을 원래 사용자 응답 생성과 라우팅에서 사용했었음
        elif mode == "messages":
            ...
            if isinstance(msg, AIMessageChunk):
                ...
                if token:
                    ...
                    yield (None, token, None, None)  # 토큰이 발생했다고 전달

    # 세션 히스토리에 저장
    if final_response:
        if session_id not in SESSION_STORE:
            SESSION_STORE[session_id] = []  # session_id별 대화가 저장
        SESSION_STORE[session_id].append(new_message)  # new_message은 사용자 질문
